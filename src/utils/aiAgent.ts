...
export function promptForReorganize(resume: Resume, userInstruction: string, standingInstructions: string): string {
  ...
  return buildFeaturePrompt('organize',
    ...
    'When rewriting bullet text, use XYZ form (verb-bank action verb + task/project + result/impact); quantify only from evidence; imply skill/tool through the work.',
...
export function promptForAgentControl(
  ...
  return buildFeaturePrompt('agent',
    ...
    `User request:\n${userMessage}`,
    'When writing or rewriting bullets, use XYZ form (verb-bank action verb + task/project + result/impact); quantify only from evidence; imply skill/tool through the work.',
    'Return ONLY valid JSON with this shape:',
